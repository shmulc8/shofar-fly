...
(function () {
	...
	function initBridge() {
		...
		fetch(metaUrl)
			...
			.then(function (buffer) {
				...
				worker = new Worker('js/sim-worker.js?v=4');
	...
	function handleWorkerMessage(e) {
		...
		case 'ready':
			...
			BRAIN.worker = worker;
			BRAIN.workerGroupNameToId = groupNameToId;
		...
		case 'plasticityReady':
			BRAIN.plasticityReady = {edges: e.data.edges, preNeurons: e.data.preNeurons};
			break;
		case 'stepped':
			BRAIN.plasticity = e.data.plasticity;
			latestFireState = e.data.fireState; BRAIN.latestFireState = e.data.fireState;
			if (BRAIN.onStepped) BRAIN.onStepped(e.data);
			break;
		case 'tick':
			BRAIN.plasticity = e.data.plasticity;
			if (BRAIN.onWorkerTick) BRAIN.onWorkerTick(e.data);
			latestFireState = e.data.fireState; BRAIN.latestFireState = e.data.fireState;
	...
	function collectOneShotSegments() {
		if (BRAIN.stimulate.labMode) { var ex = BRAIN.stimulate.extraOneShot || []; BRAIN.stimulate.extraOneShot = null; return ex; }
		var segs = [];
		if (BRAIN.stimulate.extraOneShot) {
			segs = segs.concat(BRAIN.stimulate.extraOneShot);
			BRAIN.stimulate.extraOneShot = null;
		}
	...
	function collectStimulationSegments() {
		var segs = [];
		var d = BRAIN.drives;
		// lab mode: the only input is what the experiment asks for (no drives, senses or tonic background)
		if (BRAIN.stimulate.labMode) return BRAIN.stimulate.extraSegments ? BRAIN.stimulate.extraSegments.slice() : [];
		...
		if (BRAIN.stimulate.extraSegments) {
			segs = segs.concat(BRAIN.stimulate.extraSegments);
		}

		// Tonic background activity (disabled when a page asks for a silent network)
		if (!BRAIN.stimulate.noTonic) {
	...
	function sendStimulation() {
		if (!worker) return;
		if (BRAIN.stimulate.manual) return;   // the experiment drives setStimulusState itself
